...
impl TemplateExecutor {
  pub fn new(project_name: String, config: TemplateConfig) -> Result<Self, String> {
      let current_dir = match std::env::current_dir() {
          Ok(dir) => dir,
          Err(e) => return Err(format!("failed to detect current directory. Details: {}", e.to_string())),
      };
  ...
  fn create_directories(&self) -> Result<(), String> {
      for dir_name in self.config.directories.iter() {
          ...
          return Err(format!("failed to created directory '{:?}'. Details: {}", dir, err.to_string()));
      }
  ...
  fn create_gitignore(&self) -> Result<(), String> {
      ...
      let mut file = match File::create(gitignore_path) {
          Ok(f) => f,
          Err(e) => return Err(format!("failed to create .gitignore file. Details: {}", e.to_string())),
      };

      for line in self.config.gitignore.iter() {
          match file.write(format!("{}\n", line).as_bytes()) {
              Ok(_) => continue,
              Err(e) => return Err(format!("failed to write to .gitignore file. Details: {}", e.to_string())),
          }
      }
  ...
  fn render_file_templates(&self) -> Result<(), String> {
      for file in self.config.files.iter() {
          ...
          let mut output_file = match File::create(&output_path) {
              Ok(f) => f,
              Err(e) => return Err(format!("failed to create output file '{:?}'. Details: {}", output_path, e.to_string())),
          };

          let content = match file.template.dyn_render() {
              Ok(c) => c,
              Err(e) => return Err(format!("failed to render template '{:?}'. Details: {}", output_path, e.to_string())),
          };

          if let Err(e) = output_file.write(content.as_bytes()) {
              return Err(format!("failed to write to output file '{:?}'. Details: {}", output_path, e.to_string()));
          }
      }
